...
## 🔄 大規模データセット用ループ処理 (MusicGen_Finetuning_Loop.ipynb)

Colabのディスク容量制限を回避するため、`MusicGen_Finetuning_Loop.ipynb` を使用することで、ZIPファイル単位での順次処理（解凍→学習→削除）が可能です。

### 使い方

1. **ノートブックを開く**: `MusicGen_Finetuning_Loop.ipynb` を開きます。
2. **設定**: `DRIVE_ROOT` などのパスを確認します。
3. **実行**: すべてのセルを実行します。

### 処理の流れ

1. **解凍**: `archive_batch_xxxx.zip` を1つ解凍。
2. **メタデータ更新**: 解凍されたファイルに合わせて `metadata.jsonl` のパスを自動書き換え。
3. **学習**: `audiocraft` ソルバーを実行（前回のチェックポイントから自動再開）。
4. **保存**: 学習後のチェックポイントをDriveに保存。
5. **削除**: 一時データを削除して次のバッチへ。
